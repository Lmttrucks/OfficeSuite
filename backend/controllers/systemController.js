...
exports.backupDatabaseAndBlobStorage = async (req, res) => {
    try {
        ...
        exec(dbBackupCommand, (error, stdout, stderr) => {
            ...
            const blobBackupCommand = `az storage blob download-batch -d "C:\\backups\\blobstorage" --account-name ${process.env.AZURE_STORAGE_ACCOUNT} --account-key ${process.env.AZURE_STORAGE_KEY} -s ${process.env.AZURE_STORAGE_CONTAINER}`;
